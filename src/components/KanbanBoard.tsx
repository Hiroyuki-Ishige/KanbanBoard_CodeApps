      ...
      <div className="p-2 sm:p-6 w-full min-h-screen bg-gradient-to-b from-zinc-600 to-zinc-900 flex items-center justify-center overflow-y-auto">
        ...
        <div className="flex items-center justify-center flex-col gap-4 w-full max-w-6xl">
          ...
          <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 mb-8 flex w-full max-w-lg shadow-lg rounded-lg overflow-hidden">
            ...
            {/* End of select and add button div */}
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-2 sm:gap-6 overflow-x-auto pb-6 w-full">
            {Object.keys(columns).map((columnId, columnIndex) => (
              <div
                ...
                onDrop={(e) =>
                  handleDrop(e, columnId as "todo" | "inProgress" | "done")
                }
              >
                <div
                  className={`p-4 text-white font-bold text-x1 rounded-t-md 
                  ${columnStyles[columnId as keyof typeof columnStyles].header}
                  `}
                >
                  {columns[columnId as keyof typeof columns].name}
                  <span className="ml-2 px-2 py-1 bg-zinc-700 bg-opacity-50 rounded-full text-sm">
                    {columns[columnId as keyof typeof columns].items.length}
                  </span>
                </div>
                ...
                <div className="p-3 min-h-64">
                  {columns[columnId as keyof typeof columns].items.length ===
                  0 ? (
                    <div className="text-center py-10 text-white italic text-lg">
                      Drop tasks here
                    </div>
                  ) : (
                    columns[columnId as keyof typeof columns].items.map(
                      (item, index) => (
                        <div
                          key={item.id}
                          ...
                          onDragStart={() =>
                            handleDragStart(
                              columnId as keyof typeof columns,
                              item
                            )
                          }
                        >
